...
client.on("message", msg => {
  if (msg.content === ".cmd" && msg.channel.id === "channel id to execute help command") {
    const embed = new discord.RichEmbed()
      ...
      .setColor("#CF9B10")
      .setFooter("Developed by Neyoshi");
...
client.on("message", msg => {
  if (
    msg.content === "!L4" &&
    msg.channel.id === "1207173261826330684"
  ) {
    const embed = new discord.RichEmbed()
      .setTitle("Methods")
      .setColor("#00ffff")
...
client.on("message", msg => {
  if (
    ...
    msg.channel.id === "1207173289655410849"
  ) {
...
client.on("message", msg => {
  if (
    ...
    msg.channel.id === "1207173648952336394"
  ) {
    const args = msg.content
      .slice(config.prefix.length)
      .trim()
...
client.on("message", msg => {
  if (
    ...
    msg.channel.id === "1207174041866342400"
  ) {
    const args = msg.content
      .slice(config.prefix.length)
      .trim()
